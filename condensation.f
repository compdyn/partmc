...
!         p0T, del_t_cond, VH)
!    
!    ! dvdt(i) and histot(i) are output
!    ! dvdt is growth rate of one droplet (m^3 s^{-1})
!    ! histot =  dmdt * 1d6 / e(i) (s^{-1})
!    
...
!    
!    !     Calculation of the term dm/dt according to Majeed and Wexler,
!    !     Atmos. Env. (2001). Since Eq. (7) in this paper is an implicit
!    !     equation (T_a depends on dm/dt), a Newton solver is applied.
!    
...
!    do i = 1,n_bin
!       do j = 1,MH(i)
!          !            write(6,*)'vor update ',i,j,VH(i,j,3), &
!          !                      VH(i,j,3) + dvdt(i,j) * del_t_cond
!          VH(i,j,3) =  VH(i,j,3) + dvdt(i,j) * del_t_cond
!          if (VH(i,j,3) .lt. 0.d0) VH(i,j,3) = 0.d0
!          !            write(6,*)'nach update ',i,j,VH(i,j,3)
!       enddo
!    enddo
!    
!  end subroutine cond_rate
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
