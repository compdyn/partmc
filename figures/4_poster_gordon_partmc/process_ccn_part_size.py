...
lumped_diameter_axis = pmc_log_axis(min = 1e-10,
                                    max = 1e-5,
                                    n_bin = 35 * 5)
...
for coag in [True, False]:
    ...
    for [time, filename, key] in time_filename_list:
        ...
        ncf.close()
        num_conc = 1.0 / array(particles.comp_vol)
        total_num_conc = num_conc.sum()
        critical_ss = particles.kappa_rh(env_state, const) - 1.0

        lumped_particles = lump_particles_by_diameter(particles, lumped_diameter_axis)
        lumped_num_conc = 1.0 / array(lumped_particles.comp_vol)
        lumped_critical_ss = lumped_particles.kappa_rh(env_state, const) - 1.0
