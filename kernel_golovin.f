...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
     ...
     &     N_0, V_0, rho_p, g, n_ln)
      ...
      real*8 d_0, beta_1, tau, T, rat_v, nn, b
      integer k
      ...
      if (time .eq. 0d0) then
         ...
         do k = 1,n_bin
            rat_v = 1d-6 * vv(k) / V_0
            call bessi1(2d0 * rat_v * sqrt(T), b)
            nn = N_0/(1d-6 * vv(k)) * (1d0-T)/sqrt(T)
     &           * exp(-((1d0+T) * rat_v)) * b
            n_ln(k) = pi/2d0 * (1d-6*dp(k))**3d0 * nn
            write(6,'(6a11)'), 'time', 'T', 'b', 'nn', 'n_ln', 'g'
            write(6,'(6e11.3)'), time, T, b, nn, n_ln(k), g(k)
         enddo
      endif

      do k = 1,n_bin
         g(k) = pi/6d0 * rho_p * (1d-6*dp(k))**3d0 * n_ln(k)
      enddo
