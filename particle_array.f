...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      subroutine smooth_kernel(n_bin, k, k_smooth)

      integer n_bin                 ! number of bins
      real*8 k(n_bin,n_bin)         ! kernel matrix
      real*8 k_smooth(n_bin,n_bin)  ! smooth kernel matrix

      integer i, j, jm, im, jp, ip
     ...
     &           + k(ip,j) + k(i,jp)) + 0.5d0 * k(i,j)
            if (i .eq. j) k_smooth(i,j) = 0.5d0 * k_smooth(i,j)
         enddo
      enddo
