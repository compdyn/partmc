...
program process
  ...
  logical, allocatable :: is_active(:), is_not_bc(:), is_bc(:)
  ...
  call aero_state_allocate(aero_state_bc)
  ...
  do i_index = 1,n_index
     do i_repeat = 1,n_repeat
        ...
        call aero_state_copy(aero_state, aero_state_bc)
        is_not_bc = (bc_masses == 0d0)
        is_bc = (bc_masses > 0d0)

        write(6,*)'size of is_not_bc', size(is_not_bc), size(is_bc)
        call aero_state_remove_particles(aero_state_bc, is_not_bc)
        write(6,*)'particles removed'

        is_bc = (aero_state_masses(aero_state_bc, aero_data, &
             include=(/"BC"/)) > 0)

        write(6,*) all(is_bc), aero_state_total_particles(aero_state_bc)

        times(i_index) = time

        dry_diameters = aero_state_dry_diameters(aero_state_bc, aero_data)
        num_concs = aero_state_num_concs(aero_state_bc)
        ...
        masses = aero_state_masses(aero_state_bc, aero_data)
        tot_mass_conc = sum(masses * num_concs)
        call stats_1d_add_entry(stats_tot_mass_conc, tot_mass_conc, i_index)
        
        least_create_times = aero_state_least_create_times(aero_state_bc)
	greatest_create_times = aero_state_greatest_create_times(aero_state_bc) 
...
!        crit_rhs = aero_state_crit_rel_humids(aero_state_bc, aero_data, &
!             env_state)
!        scs = crit_rhs - 1d0
...
!> composition-averaging
        call aero_state_copy(aero_state_bc, aero_state_averaged)
...
!> ratio of entropy conc. and entropy conc. after composition-averaging (R)
        ...
        write(6,*)'analysis done'

        call make_filename(out_filename, prefix, "_process_bc.nc", index, repeat)
...
!        call pmc_nc_write_real_1d(ncid, so4_fracs, "so4_frac", dim_name="particle", unit="1")
     ...
     write(6,*)'done with repeat loop'
     call make_filename(out_filename, prefix, "_process_bc.nc", index)
     call pmc_nc_open_write(out_filename, ncid)
     call pmc_nc_write_info(ncid, uuid, "1_urban_plume process")
...
!     call stats_2d_clear(stats_diam_sc_dist)
  ...
  call make_filename(out_filename, prefix, "_process_bc.nc")
  call pmc_nc_open_write(out_filename, ncid)
  call pmc_nc_write_info(ncid, uuid, "1_urban_plume process")
...
!       unit="m^{-3}")  
  ...
  call aero_state_deallocate(aero_state_bc)
