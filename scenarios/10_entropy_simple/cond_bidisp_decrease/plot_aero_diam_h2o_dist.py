...
for (filename, index) in partmc.get_filename_list('out/', r'urban_plume_([0-9]+)_process\.nc'):
    ...
    h2o_frac_edges = ncf.variables["h2o_frac_edges"].data * 100
    diam_h2o_dist = ncf.variables["diam_h2o_dist"].data * 1e-6

    p = axes.pcolor(diam_edges, h2o_frac_edges, diam_h2o_dist,
    ...
    axes.set_ylabel(r"H2O mass fraction $w_{\rm H2O}$ / \%")
    axes.set_ylim(0, 200)
    ...
    cbar.set_label(r"number conc. $n(D_{\rm dry},w_{\rm H2O})$ / $\rm cm^{-3}$")

    out_filename = "out/urban_plume_diam_h2o_dist_%s.pdf" % index
