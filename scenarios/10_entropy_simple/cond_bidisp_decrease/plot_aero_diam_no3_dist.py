...
for (filename, index) in partmc.get_filename_list('out/', r'urban_plume_([0-9]+)_process\.nc'):
    ...
    no3_frac_edges = ncf.variables["no3_frac_edges"].data * 100
    diam_no3_dist = ncf.variables["diam_no3_dist"].data * 1e-6

    p = axes.pcolor(diam_edges, no3_frac_edges, diam_no3_dist,
    ...
    cbar.set_label(r"number conc. $n(D_{\rm dry},w_{\rm NO_3})$ / $\rm cm^{-3}$")

    out_filename = "out/urban_plume_diam_no3_dist_%s.pdf" % index
