...
program process
  ...
  type(stats_1d_t) :: stats_tot_num_conc, stats_tot_mass_conc, &
       stats_tot_entropy, stats_tot_entropy_averaged
  ...
  do i_index = 1,n_index
     do i_repeat = 1,n_repeat
        ...
        call stats_1d_add(stats_num_dist, &
             bin_grid_histogram_1d(diam_grid, dry_diameters, num_concs))
        call stats_2d_add(stats_diam_bc_dist, &
             bin_grid_histogram_2d(diam_grid, dry_diameters, &
             bc_grid, bc_fracs, num_concs))
        call stats_2d_add(stats_diam_sc_dist, &
             bin_grid_histogram_2d(diam_grid, dry_diameters, &
             sc_grid, scs, num_concs))
        call stats_1d_add(stats_entropy_dist, &
             bin_grid_histogram_1d(entropy_grid, entropies, &
             num_concs))
     end do
