...
for (filename, index) in partmc.get_filename_list('out/', r'urban_plume_([0-9]+)_process\.nc'):
    ...
    diam_coating_dist = ncf.variables["diam_coating_dist"].data * 1e-6

    p = axes.pcolor(diam_edges, coating_edges, diam_coating_dist,
    ...
    axes.set_ylabel(r"coating thickness / nm")
    axes.set_ylim(0, 10)
    ...
    cbar.set_label(r"number conc. $n(D_{\rm dry}, \ell)$ / $\rm cm^{-3}$")

    out_filename = "out/urban_plume_diam_coating_dist_%s.pdf" % index
