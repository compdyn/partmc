...
for (filename, index) in partmc.get_filename_list('out/', r'urban_plume_([0-9]+)_process\.nc'):
    ...
    diam_gamma_dist = ncf.variables["diam_gamma_dist"].data * 1e-6

    p = axes.pcolor(diam_edges, gamma_edges, diam_gamma_dist,
                    norm = matplotlib.colors.LogNorm(vmin=1e4, vmax=1e8), linewidths = 0.1)
    ...
    out_filename = "out/urban_plume_diam_gamma_org_dist_%s.pdf" % index
