...
plot "out/chamber_00000001_num_dist.txt" using ($1*1e9):($2/1e6):($3/1e6) with errorbars title "PartMC simulation, t = 0 min", \
     ...
     "ref_0925/ref_aero_size_num.txt" using ($1*1e9):($2/1e6) with lines title "barrel experiment 1, t = 0 min", \
     "ref_0925/ref_aero_size_num.txt" using ($1*1e9):($3/1e6) with lines title "barrel experiment 1, t = 7 min", \
     "ref_0925/ref_aero_size_num.txt" using ($1*1e9):($12/1e6) with lines title "barrel experiment 1, t = 70 min", \
     "ref_0925/ref_aero_size_num.txt" using ($1*1e9):($32/1e6) with lines title "barrel experiment 1, t = 210 min"
