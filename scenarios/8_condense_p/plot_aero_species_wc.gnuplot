...
plot "out/condense_0001_aero_time.txt" using ($1):($5*1e9) axes x1y1 with lines title "NO3", \
     "out/condense_0001_aero_time.txt" using ($1):($21*1e9) axes x1y1 with lines title "OC", \
     "out/condense_0001_aero_time.txt" using ($1):($7*1e9) axes x1y1 with lines title "NH4"

plot "out/condense_0001_aero_time.txt" using ($1):($4*1e9) axes x1y1 with lines title "SO4", \
     "out/condense_0001_aero_time.txt" using ($1):($22*1e9) axes x1y1 with lines title "BC", \
     "out/condense_0001_aero_time.txt" using ($1/3600):(($9+$10+$11+$12+$13+$14+$15+$16)*1e9) axes x1y1 with lines title "SOA" # ARO1 + ARO2 + ALK1 + OLE1 + API1 + API2 + LIM1 + LIM2
