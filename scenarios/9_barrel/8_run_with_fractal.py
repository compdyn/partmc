...
if not os.path.exists("spec_with_fractal"):
    os.mkdir("spec_with_fractal")

if not os.path.exists("out_with_fractal"):
       os.mkdir("out_with_fractal")
...
case = 0
for frac_dim in arange(1.8,3.1,0.1):
    filename_in = "barrel_template_with_fractal.spec"
    case += 1
    filename_out = "spec_with_fractal/barrel_wc_case_%04d.spec" % (case)
    f_in = open(filename_in, 'r')
    f_out = open(filename_out, 'w')

    for line in f_in:
        line = line.replace('%%OUTPUT_PREFIX%%', 'out_with_fractal/case_%04d_wc' % (case))
...
filename_out_num = "rel_err_num_w_fractal.dat"
f_out_num = open(filename_out_num, 'w')
f_out_num.write("# Colume 1: caseID\n")
f_out_num.write("# Colume 2: fractal dimension\n")
f_out_num.write("# Colume j+2: relative error of number distribution at time(j)\n")
filename_out_mass = "rel_err_mass_w_fractal.dat"
f_out_mass = open(filename_out_mass, 'w')
f_out_mass.write("# Colume 1: caseID\n")
f_out_mass.write("# Colume 2: fractal dimension\n")
f_out_mass.write("# Colume j+2: relative error of mass distribution at time(j)\n")
filename_out_total = "rel_err_total_w_fractal.dat"
f_out_total = open(filename_out_total, 'w')
f_out_total.write("# Colume 1: caseID\n")
f_out_total.write("# Colume 2: fractal dimension\n")
f_out_total.write("# Colume j+2: total relative error (num + mass) at time(j)\n")
filename_out_rmse_num = "rmse_num_w_fractal.dat"
f_out_rmse_num = open(filename_out_rmse_num, 'w')
f_out_rmse_num.write("# Colume 1: caseID\n")
f_out_rmse_num.write("# Colume 2: fractal dimension\n")
f_out_rmse_num.write("# Colume 3: root mean square error of number distribution\n")
filename_out_rmse_mass = "rmse_mass_w_fractal.dat"
f_out_rmse_mass = open(filename_out_rmse_mass, 'w')
f_out_rmse_mass.write("# Colume 1: caseID\n")
f_out_rmse_mass.write("# Colume 2: fractal dimensionr\n")
f_out_rmse_mass.write("# Colume 3: root mean square error of mass distribution\n")
filename_out_rmse_total = "rmse_total_w_fractal.dat"
f_out_rmse_total = open(filename_out_rmse_total, 'w')
f_out_rmse_total.write("# Colume 1: caseID\n")
f_out_rmse_total.write("# Colume 2: fractal dimension\n")
f_out_rmse_total.write("# Colume 3: total root mean square error (num + mass)\n")

# run simulations, store the errors
path="spec_with_fractal/"
dirList=os.listdir(path)
case = 0
for fname in dirList:

    case += 1
    spec_file = "spec_with_fractal/barrel_wc_case_%04d.spec" % (case)

    frac_dim = read_values_from_spec_file(spec_file, 'frac_dim')

    ncfile_prefix = "out_with_fractal/case_%04d_wc_0001" % (case)
    ...
    f_out_num.write("%04d   %.1f    " % (case, frac_dim))
    f_out_mass.write("%04d   %.1f    " % (case, frac_dim))
    f_out_total.write("%04d   %.1f   " % (case, frac_dim))
    f_out_rmse_num.write("%04d   %.1f    " % (case, frac_dim))
    f_out_rmse_mass.write("%04d   %.1f    " % (case, frac_dim))
    f_out_rmse_total.write("%04d   %.1f   " % (case, frac_dim))
...
data5 = genfromtxt("rmse_num_w_fractal.dat", skip_header=3)
data6 = genfromtxt("rmse_mass_w_fractal.dat", skip_header=3)
data7 = genfromtxt("rmse_total_w_fractal.dat", skip_header=3)
