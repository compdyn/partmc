...
barrel = np.loadtxt("ref_0925/ref_aero_time.txt")
(figure, axes) = mpl_helper.make_fig(colorbar=False)
axes.plot(partmc_only_coag[:,0]/60, partmc_only_coag[:,1], color='g')
axes.plot(partmc_only_wall[:,0]/60, partmc_only_wall[:,1], color='b')
axes.plot(partmc_coag_wall[:,0]/60, partmc_coag_wall[:,1], color='k')
axes.plot(barrel[:,0]/60, barrel[:,1], color='r')
axes.set_xlabel("Time (min)")
axes.set_ylabel(r"Number concentration ($\mathrm{m}^{-3}$)")
axes.grid()
#axes.set_yscale('log')
axes.legend(('PartMC (Only coag.)', 'PartMC (Only wall)', 'PartMC (Coag.+wall)', 'Barrel'), loc='best')
