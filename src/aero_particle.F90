...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  real(kind=dp) function aero_particle_crit_diameter_est(&
    ...
    real(kind=dp) :: kappa, dry_diam, A
    real(kind=dp) :: surf_eng_soluble, surf_eng_organic
    real(kind=dp) :: v_solid, v_sol, v_org, r_solid
    real(kind=dp) :: d, v_delta_solid, r_core, v_delta, d_v_delta
    real(kind=dp) :: d_eff_surf_eng, d_2_eff_surf_eng, R, d_R, f, df, dd
    ...
    if (v_solid == 0d0) then
      do i_newton = 1, CRIT_DIAM_MAX_ITER
        surf_eng_soluble = aero_particle_surf_eng_soluble(aero_particle, &
        ...
        if (v_org > v_delta) then
          eff_surf_eng = surf_eng_organic
          f = d**6 - c_1 * d**4 / eff_surf_eng - c_2 * d**3 + c_3
          df = 6d0 * d**5 - 4d0 * c_1 * d**3 / eff_surf_eng - 3d0 * c_2 * d**2
        else
          if (v_org == 0) then
            eff_surf_eng = surf_eng_soluble
            f = d**6 - c_1 * d**4 / eff_surf_eng - c_2 * d**3 + c_3
            df = 6d0 * d**5 - 4d0 * c_1 * d**3 / eff_surf_eng - 3d0 * c_2 * d**2
          else
            eff_surf_eng = surf_eng_soluble + c_5 / v_delta
            d_v_delta = 2d0 * const%pi * delta_min * (d - delta_min)
            d_eff_surf_eng = - c_5 * d_v_delta / v_delta**2
            R = eff_surf_eng - d * d_eff_surf_eng
            d_2_eff_surf_eng = (c_5 / v_delta**3) * (2d0 * d_v_delta**2 - v_delta * c_4)
      ...
      do i_newton = 1, CRIT_DIAM_MAX_ITER
        ...
        if (v_sol + v_org > v_delta_solid) then
          ...
          eff_surf_eng = (v_org * surf_eng_organic + v_sol * surf_eng_soluble) &
               / v_delta_solid
          d = dry_diam
          exit
        end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  !> Returns the volume-weighted surface tension of the soluble core
  !> (soluble inorganics plus water) at diameter d (J m^-2).
  real(kind=dp) function aero_particle_surf_eng_soluble(aero_particle, &
       aero_data, env_state, d)
    ...
    !> Environment state.
    type(env_state_t), intent(in) :: env_state
    !> Diameter at which to evaluate the surface tension (m).
    real(kind=dp) :: d

    integer :: i_sol_spec, n_sol_spec
    real(kind=dp) :: aero_particle_soluble_volume
    ...
    aero_particle_surf_eng_soluble = 0d0
    do n_sol_spec = 1, size(sol_spec)
      i_sol_spec = aero_data_spec_by_name(aero_data, sol_spec(n_sol_spec))
      aero_particle_surf_eng_soluble = aero_particle_surf_eng_soluble &
              + aero_particle%vol(i_sol_spec) * &
              aero_data%sigma(i_sol_spec) / v_sol
    end do
    aero_particle_surf_eng_soluble = aero_particle_surf_eng_soluble + v_water &
              * const%water_surf_eng / v_sol

  end function aero_particle_surf_eng_soluble
