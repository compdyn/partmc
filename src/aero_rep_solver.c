            ...
            int *deriv_ids, int **jac_ids)
{
  ...
  // Get a pointer to the aerosol representation data
  int *aero_rep_data = (int*) (model_data.aero_rep_data);
    ...
    switch (aero_rep_type) {
      case AERO_REP_MODAL_BINNED_MASS :
        aero_rep_data = (int*) aero_rep_modal_binned_mass_update_ids(
                  model_data, deriv_ids, jac_ids, (void*) aero_rep_data);
        break;
      case AERO_REP_SINGLE_PARTICLE :
          ...
          void *solver_data)
{
  ...
  for( int i_state = 0; i_state < sd->n_states; i_state++ ) {
    ...
    int *aero_rep_data = (int*) (md->nxt_aero_rep);
            ...
            void *update_data, void *solver_data)
{
  ModelData *model_data = (ModelData*)
          &(((SolverData*)solver_data)->model_data[state_id]);
...
void aero_rep_print_data(void *solver_data)
{
  ...
  printf("\n\nAerosol representation data for state %d\n\nnumber of aerosol "
          "representations: %d\n\n", i_state, n_aero_rep);
