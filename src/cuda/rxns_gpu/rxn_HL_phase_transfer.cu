 ...
 */
void * rxn_gpu_HL_phase_transfer_get_used_jac_elem(ModelDatagpu *model_data,
          void *rxn_data, bool **jac_struct)
{
  ...
  for (int i_aero_phase = 0; i_aero_phase < NUM_AERO_PHASE_; i_aero_phase++) {
    ...
    if (i_used_elem != n_jac_elem) {
      ...
      rxn_gpu_HL_phase_transfer_print(rxn_data);
      exit(1);
    }
 ...
 * \return The rxn_data pointer advanced by the size of the reaction data
 */
void * rxn_gpu_HL_phase_transfer_update_ids(ModelDatagpu *model_data, int *deriv_ids,
 ...
 * \return The rxn_data pointer advanced by the size of the reaction data
 */
void * rxn_gpu_HL_phase_transfer_update_env_state(double *env_data,
          void *rxn_data)
{
  int *int_data = (int*) rxn_data;
  double *float_data = (double*) &(int_data[INT_DATA_SIZE_]);
 ...
 */
void * rxn_gpu_HL_phase_transfer_scale_adj(ModelDatagpu *model_data, void *rxn_data)
{
  realtype *state = model_data->state;
  realtype *env_data = model_data->env;
  int *int_data = (int*) rxn_data;
  ...
  for (int i_phase=0; i_phase<NUM_AERO_PHASE_; i_phase++) {
    ...
    //if (DERIV_ID_(1+i_phase)>=0) deriv[DERIV_ID_(1+i_phase)] +=
      ...
      if (DERIV_ID_(1+i_phase)>=0)
            atomicAdd((float*)&(deriv[DERIV_ID_(1+i_phase)]),cond_rate / UGM3_TO_PPM_ - evap_rate);
