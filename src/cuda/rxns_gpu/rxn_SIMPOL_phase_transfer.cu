 ...
 */
void * rxn_gpu_SIMPOL_phase_transfer_get_used_jac_elem(void *rxn_data,
          bool **jac_struct)
{
 ...
 * \return The rxn_data pointer advanced by the size of the reaction data
 */
void * rxn_gpu_SIMPOL_phase_transfer_update_ids(ModelDatagpu *model_data,
 ...
 * \return The rxn_data pointer advanced by the size of the reaction data
 */
void * rxn_gpu_SIMPOL_phase_transfer_pre_calc(ModelDatagpu *model_data, void *rxn_data,
          double time_step)
{
 ...
 */
void * rxn_gpu_SIMPOL_phase_transfer_scale_adj(ModelDatagpu *model_data, void *rxn_data)
{
  realtype *state = model_data->state;
  realtype *env_data = model_data->env;
  int *int_data = (int*) rxn_data;
          ...
          realtype *deriv, void *rxn_data, double time_step)
{
  ...
  for (int i_phase=0; i_phase<NUM_AERO_PHASE_; i_phase++) {
    ...
    if (DERIV_ID_(0)>=0) atomicAdd((float*)&(deriv[DERIV_ID_(0)]),evap_rate - cond_rate);
    ...
    if (DERIV_ID_(1+i_phase)>=0) {
      if (aero_conc_type==0) {
        ...
        atomicAdd((float*)&(deriv[DERIV_ID_(1+i_phase)]),(cond_rate - evap_rate) /
          UGM3_TO_PPM_ / number_conc);
 ...
 */
#ifdef PMC_USE_SUNDIALS
void * rxn_gpu_SIMPOL_phase_transfer_calc_jac_contrib(ModelDatagpu *model_data,
