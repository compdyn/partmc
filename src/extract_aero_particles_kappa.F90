...
!> masses.
program extract_aero_particles_kappa
  ...
  if (out_filename == "") then
     ...
     if (i_char == 0) then
        out_filename = trim(in_filename) // "_aero_particles_kappa.txt"
     else
        out_filename = trim(in_filename(1:(i_char - 1))) &
             // "_aero_particles_kappa.txt"
     end if
  end if
  ...
  write(*,'(a)') "    column  2: particle dry diameter (m)"
  write(*,'(a)') "    column  3: particle kappa "
  ...
  do i_part = 1,aero_state%apa%n_part
     ...
     write(out_unit, '(i15,e30.15e3,e30.15e3,e30.15e3)', advance='no') &
          aero_particle%id, &
          aero_particle_dry_diameter(aero_particle, aero_data), &
...
end program extract_aero_particles_kappa
