 ...
 *
 * Used to hold data required to solve a single model state 
 */
typedef struct {
 ...
 *
 * Used for solver data of entire system (i.e. all states being solved)
 */
typedef struct {
#ifdef PMC_USE_SUNDIALS
  ...
  SUNMatrix J_init;	        // sparse Jacobian matrix with elements
                                //      initialized to 1.0
#endif
  void *cvode_mem;	        // CVodeMem object
  int n_states;                 // number of states to solve simultaneously
  ModelData *model_data;        // Model data array - one element for each state being solved
...
void sub_model_update_data(int state_id, int update_sub_model_type, void *update_data,
          void *solver_data);
int sub_model_get_parameter_id_sd(void *solver_data, int state_id, int sub_model_type,
          void *identifiers);
...
void * aero_rep_get_effective_radius(ModelData model_data,
          int aero_rep_idx, int aero_phase_idx, PMC_C_FLOAT *radius);
void * aero_rep_get_number_conc(ModelData model_data,
...
void * aero_rep_get_aero_phase_mass(ModelData model_data,
