          ...
          void *rxn_data)
{
  ...
  PMC_C_FLOAT vp = B1_ / TEMPERATURE_K_
          ...
          PMC_SOLVER_C_FLOAT *deriv, void *rxn_data, PMC_C_FLOAT time_step)
{
  ...
  for (int i_phase=0; i_phase<NUM_AERO_PHASE_; i_phase++) {
    ...
    // aerosol phase (1/s)
    PMC_C_FLOAT cond_rate = number_conc/(radius*radius/(3.0*DIFF_COEFF_) + 
              4.0*radius/(3.0*C_AVG_ALHPA_));
  
    // Calculate the evaporation rate constant (ppm_x*m^3/ug_x/s)
    PMC_C_FLOAT evap_rate = cond_rate * (EQUIL_CONST_ * aero_phase_avg_MW / 
              aero_phase_mass);
    ...
    if (DERIV_ID_(0)>=0) deriv[DERIV_ID_(0)] += (PMC_SOLVER_C_FLOAT) (
              evap_rate - cond_rate );
    ...
    if (DERIV_ID_(1+i_phase)>=0) { 
      if (aero_conc_type==0) {
        ...
        deriv[DERIV_ID_(1+i_phase)] += (PMC_SOLVER_C_FLOAT) (
                (cond_rate - evap_rate) / UGM3_TO_PPM_ / number_conc );
      } else {
        // Total aerosol mass condensation
        deriv[DERIV_ID_(1+i_phase)] += (PMC_SOLVER_C_FLOAT) (
                (cond_rate - evap_rate) / UGM3_TO_PPM_ );
      }
    }
  }
          ...
          PMC_SOLVER_C_FLOAT *J, void *rxn_data, PMC_C_FLOAT time_step)
{
  ...
  for (int i_phase=0; i_phase<NUM_AERO_PHASE_; i_phase++) {
    ...
    if (JAC_ID_(1+i_phase*3+1)>=0) 
            J[JAC_ID_(1+i_phase*3+1)] += (PMC_SOLVER_C_FLOAT) evap_rate;
    ...
    if (aero_conc_type==0) {
      // Per-particle condensation
      if (JAC_ID_(1+i_phase*3)>=0) J[JAC_ID_(1+i_phase*3)] += (PMC_SOLVER_C_FLOAT) (
              cond_rate / number_conc / UGM3_TO_PPM_ );
      if (JAC_ID_(1+i_phase*3+2)>=0) J[JAC_ID_(1+i_phase*3+2)] -= (PMC_SOLVER_C_FLOAT) (
              evap_rate / number_conc / UGM3_TO_PPM_ ); 
    } else {
      // Total aerosol mass condensation
      if (JAC_ID_(1+i_phase*3)>=0) J[JAC_ID_(1+i_phase*3)] += (PMC_SOLVER_C_FLOAT) (
