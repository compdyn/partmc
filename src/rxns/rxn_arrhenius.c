 ...
 */
void * rxn_arrhenius_get_used_jac_elem(void *rxn_data, pmc_bool **jac_struct)
{
 ...
 * \return The rxn_data pointer advanced by the size of the reaction data
 */
void * rxn_arrhenius_update_env_state(PMC_C_FLOAT *env_data, void *rxn_data)
{
  int *int_data = (int*) rxn_data;
  PMC_C_FLOAT *float_data = (PMC_C_FLOAT*) &(int_data[INT_DATA_SIZE_]);
          ...
          PMC_SOLVER_C_FLOAT *deriv, void *rxn_data, PMC_C_FLOAT time_step)
{
  ...
  if (rate!=ZERO) {
    ...
    for (int i_spec=0; i_spec<NUM_PROD_; i_spec++, i_dep_var++) {
      if (DERIV_ID_(i_dep_var) < 0) continue; 
      deriv[DERIV_ID_(i_dep_var)] += (PMC_SOLVER_C_FLOAT) rate*YIELD_(i_spec);
    }
  }
 ...
 */
#ifdef PMC_USE_SUNDIALS
void * rxn_arrhenius_calc_jac_contrib(ModelData *model_data, PMC_SOLVER_C_FLOAT *J,
