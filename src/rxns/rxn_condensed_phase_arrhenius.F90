...
!> \page phlex_rxn_condensed_phase_arrhenius Phlexible Mechanism for Chemistry: Phase-Transfer Reaction
!!
!! Condensed-phase Arrhenius reactions are calculated as forward and reverse
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine initialize(this, chem_spec_data, aero_rep)
    ...
    call assert_msg(583327500, &
            ...
            "Missing aerosol-phase water in condensed-phase Arrhenius reaction")
    ...
    call assert_msg(191050890, associated(aero_rep), &
            "Missing aerosol representation for condensed phase Arrhenius reaction")
    ...
    call assert_msg(519823533, &
            ...
            "Missing 'k_reverse' for condensed phase Arrhenius reaction")
    key_name = "time_unit"
    ...
    do i_aero_rep = 1, size(aero_rep)
      ...
      if (ion_pair_name.ne."") then
        ...
        call assert_msg(298310186, size(unique_names).eq.num_phase, &
                ...
                "' in phase '"//phase_name// &
                "' in an condensed phase Arrhenius reaction")
        ...
        call assert_msg(450743055, tracer_type.eq.CHEM_SPEC_ACTIVITY_COEFF, &
                ...
                "'ION_PAIR' to be used as an ion pair in a "// &
      ...
      do i_spec = 1, _NUM_REACT_
        ...
        call assert_msg(280008989, size(unique_names).eq.num_phase, &
                ...
                "' in phase '"//phase_name//"' in an condensed phase Arrhenius reaction")
