...
#define _RATE_CONSTANT_ float_data[9]
#define _NUM_INT_PROP_ 2
#define _NUM_FLOAT_PROP_ 10
...
void * rxn_troe_update_env_state(double *env_data, void *rxn_data)
{
  ...
  // k = ([M]*k0 / (1 + k0/kinf)) * Fc^(1 / (1/N + log(k0/kinf)^2))
  ...
  _RATE_CONSTANT_ = (k0 / (ONE + kinf))
	  * SUNRpowerR(_Fc_, (ONE / (ONE/_N_ + SUNRpowerI((log10(kinf)),2))))
 ...
 * \return The rxn_data pointer advanced by the size of the reaction data
 */
void * rxn_troe_calc_jac_contrib(double *state, double *J,
		void *rxn_data)
{
  int *int_data = (int*) rxn_data;
  realtype *float_data = (realtype*) &(int_data[_INT_DATA_SIZE_]);
