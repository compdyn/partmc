...
! $Header: /project/work/rep/CCTM/src/chem/ebi_cb05cl_ae5/hrg1.F,v 1.1.1.1 2008/02/08 21:09:23 sjr Exp $ 

! what(1) key, module and SID; SCCS file; date and time of last delta:
! %W% %P% %G% %U%
!OJORBA3
       SUBROUTINE EXT_HRG1( DTC )
!OJORBA3
!**********************************************************************
!
!  FUNCTION: To solve for the concentration of NO2, NO, O3, and O3P
!            algebraically.
!
...
!  O1D Section
!    1) sum of the rate constants for all O1D loss reactions
!    2) get fractional yield of O3P from O1D loss
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      O1D_S =            RKI(  10 )                   ! O1D=O
     &        +          RKI(  11 )                   ! O1D=2*OH
     &        +          RKI(  38 )                   ! O1D=OH+HO2
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!  NO Section
!    R1_2 = production of NO from NO2 ( rates of form k[NO2][x] )
!           except NO2+NO3=NO+NO2 (it is treated as if it were NO3=NO )
!    P1 =   remaining NO production terms
!    L1 =   loss of NO (except rxns producing NO2 - they are in R2_1)
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      R1_2  =            RKI(   1 )                   ! NO2=NO+O
     &        +          RKI(   4 ) * YC( O       )   ! NO2+O=NO
     &        +  0.200 * RKI( 156 ) * YC( ISOP    )   ! NO2+ISOP=0.2*NO+...
      R1_2  = R1_2 * DTC
     ...
     &        +  0.100 * RKI( 129 ) * YC( TO2     )   ! NO+TO2=0.9*NO2+...
      L1    = 1.0 + L1 * DTC


!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!  NO2 Section
!    R2_1 = production of NO2 from NO ( rates of form k[NO][x] )
!            a)  NO+O3=NO2 not included
!            b)  NO+NO3=2NO2 ( 1/2 of NO2 formation rate included )
!            c)  NO3+NO2=NO+NO2 is not included for NO2
!    P2 =  remaining NO2 production terms 
!            a)  NO+O3=NO2 not included
!            b)  NO+NO3=2NO2 (1/2 of NO2 formation rate included )
!    L2 = loss of NO2 (except rxns producing NO2 - they are in R1_2)
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &        +  0.900 * RKI( 129 ) * YC( TO2     )   ! NO+TO2=0.9*NO2+...
     &        +          RKI( 161 ) * YC( CLO     )   ! NO+CLO=NO2+CL
      R2_1  = R2_1 * DTC
     ...
     &    +  0.200 * RXRAT( 144 )      ! NO3+ISOP=0.2*NO2+...
     &    +  0.470 * RXRAT( 152 )      ! TERP+NO3=0.47*NO2+...
      P2 = YC0( NO2 ) + P2 * DTC
     ...
     &        +          RKI( 133 ) * YC( CRO     )   ! NO2+CRO=NTR
     &        +  0.800 * RKI( 156 ) * YC( ISOP    )   ! NO2+ISOP=0.2*NO+...
      L2     = 1.0 + L2 * DTC


!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!  O3 Section
!    P3 = production of O3 except O+O2=O3
!    L3 =   loss terms for O3 except NO+O3=NO2
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &        +          RKI( 137 ) * YC( OPEN    )   ! O3+OPEN=...
     &        +          RKI( 143 ) * YC( ISOP    )   ! O3+ISOP=...
     &        +          RKI( 146 ) * YC( ISPD    )   ! O3+ISPD=...
     &        +          RKI( 151 ) * YC( TERP    )   ! O3+TERP=0.57*OH+...
     &        +          RKI( 159 ) * YC( CL      )   ! O3+CL=CLO
      L3    = 1.0 + L3 * DTC


!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!  O3P Section 
!    P12 = production of O3P except NO2+hv=O3P (J1)
!    L12 = loss terms
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      P12 =           RXRAT(   8 )      ! O3=O
     ...
     &        +          RKI( 141 ) * YC( ISOP    )   ! O+ISOP=0.75*ISPD+...
     &        +          RKI( 149 ) * YC( TERP    )   ! O+TERP=0.15*ALDX+...
      L12   = 1.0 + L12 * DTC
...
!..compute reciprocal of loss terms
      L1_INV  = 1.0 / L1
      L2_INV  = 1.0 / L2
      L3_INV  = 1.0 / L3
      L12_INV = 1.0 / L12

!..compute specific k*delta t terms
      RK1 = RKI(   1 ) * DTC            ! J1    (NO2+hv=NO+O3P)
      RK2 = RKI(   2 ) * DTC            ! J2    (O3P+O2=O3)
      RK3 = RKI(   3 ) * DTC            ! k1_3  (NO+O3=NO2)

!..compute terms that are used to calulate a,b & c
...
!       [NO]   = ( P1 + x ) / ( 1 + L1 )
!       [NO2]  = ( P2 - x ) / ( 1 + L2 )
!       [O3 ]  = ( P3 + Ko3p->O3 ) / (1 + K1,3 * [NO] + L3 )
!       [O3P]  = ( P12 + J1 * [NO2] ) / ( 1 + L12 )
!       [O1D2] = ( Ko3->o1d * [O3] ) / ( O1D_DNM )
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
