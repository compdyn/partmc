...
program pmc_test_cb05cl_ae5
  ...
  integer(kind=i_kind), parameter :: NUM_TIME_STEPS = 1!100
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  logical function run_standard_cb05cl_ae5_test() result(passed)
    ...
    do i_photo_rxn = 1, n_photo_rxn
      ...
      call rate_update(i_photo_rxn)%set_rate(real(new_rates(i_photo_rxn)/60, kind=dp))
    ...
    open(30, &
    file="../../../../test/chemistry/cb05cl_ae5/files/ebi_input_1_1_9_ksskse_photo0_lemisF_ldrydepF_lcldchemF.txt",&
    status="old")    ! Get ebi in MONARCH order
