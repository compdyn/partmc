...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  logical function run_HL_phase_transfer_test()
    ...
    integer(kind=phlex_int) :: idx_phase, idx_aero_rep, idx_O3, idx_O3_aq, &
            idx_H2O2, idx_H2O2_aq, idx_H2O_aq, i_time, i_spec, i_state
    ...
    if (pmc_mpi_rank().eq.1) then
      ...
      do i_state = 1, NUM_STATE
        call phlex_core%update_aero_rep_data(i_state, radius_update)
        call phlex_core%update_aero_rep_data(i_state, number_update)
      end do
