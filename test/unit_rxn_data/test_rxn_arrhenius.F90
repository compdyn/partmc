...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  logical function run_arrhenius_test()
    ...
    real(kind=phlex_real), dimension(0:NUM_TIME_STEP, 4) :: model_conc, true_conc
    integer(kind=phlex_int) :: idx_A, idx_B, idx_C, idx_D
    character(len=:), allocatable :: key
    integer(kind=phlex_int) :: i_time, i_spec, i_state
    real(kind=phlex_real) :: conc_D, time_step, time
#ifdef PMC_USE_MPI
    ...
    real(kind=phlex_real) :: k1, k2, temp, pressure, conv
    ...
    if (pmc_mpi_rank().eq.1) then
      ...
      ! Set the environmental conditions for all states
