...
!> \file
!> The pmc_test_wennberg_tunneling program

!> Test of wennberg_tunneling reaction module
program pmc_test_wennberg_tunneling
  ...
  if (run_wennberg_tunneling_tests()) then
    if (pmc_mpi_rank().eq.0) write(*,*) "Wennberg tunneling reaction tests - PASS"
  else
    if (pmc_mpi_rank().eq.0) write(*,*) "Wennberg tunneling reaction tests - FAIL"
    stop 3
  end if
...
end program pmc_test_wennberg_tunneling
