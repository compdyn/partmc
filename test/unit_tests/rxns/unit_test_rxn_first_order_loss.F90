...
!> \file
!> The pmc_unit_test_rxn_first_order_loss module
module pmc_unit_test_rxn_first_order_loss
  ...
  !> The first_order_loss rxn unit test
  type, extends(unit_test_data_t) :: unit_test_rxn_first_order_loss_t
    private
    !> Species ids
    integer(kind=i_kind) :: idx_A, idx_B
  ...
  end type unit_test_rxn_first_order_loss_t

  !> Constructor for the first_order_loss test
  interface unit_test_rxn_first_order_loss_t
    procedure :: constructor
  end interface unit_test_rxn_first_order_loss_t

contains

  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !> Constructor for the first_order_loss unit test
  function constructor() result(new_obj)

    !> A new unit test object
    type(unit_test_rxn_first_order_loss_t), pointer :: new_obj
  ...
  subroutine initialize( this, camp_core )
    ...
    !> Unit test data
    class(unit_test_rxn_first_order_loss_t), intent(inout) :: this
    !> CAMP core
    ...
    integer(kind=i_kind) ::  i_mech_rxn_A, i_mech_rxn_B, i_rxn
    real(kind=dp) :: rate_A, rate_B
    ...
    ! Find the A first_order_loss reaction
    key = "rxn id"
    i_mech_rxn_A = 0
    i_mech_rxn_B = 0
  ...
  function input_file_name(this)
    ...
    !> Unit test data
    class(unit_test_rxn_first_order_loss_t), intent(in) :: this

    input_file_name = "rxn_first_order_loss_config.json"
  ...
  function output_file_name(this)
    ...
    output_file_name = "rxn_first_order_loss_results.txt"
  ...
  subroutine initialize_state(this, grid_cell_id, camp_core, camp_state, &
    ...
    call this%rate_update_A%set_rate(rate_A)
    call this%rate_update_B%set_rate(432.3d0)
    call camp_core%update_rxn_data(this%rate_update_A)
    call camp_core%update_rxn_data(this%rate_update_B)

    ! Test re-setting of the rxn B rate
    call this%rate_update_B%set_rate(rate_B)
    call camp_core%update_rxn_data(this%rate_update_B)
  ...
  function analyze_or_output(this, camp_core, camp_state, unique_state_id, &
    ...
    real(kind=dp) :: k1, k2, conv, rate_A, rate_B, time
    ...
    !todo check if this works now (true_conc was wrong and now is theorically correct)
    true_conc( 1 ) = init_conc( 1 ) * exp(-(k1)*time)
    true_conc( 2 ) = init_conc( 2 ) * exp(-(k2)*time)
  ...
  integer(kind=i_kind) function pack_size(this, comm)
    ...
    pack_size = &
            pmc_mpi_pack_size_integer(this%idx_A, l_comm) + &
                    ...
                    this%rate_update_A%pack_size() + &
                    this%rate_update_B%pack_size()
#else
    pack_size = 0
#endif
  ...
  subroutine bin_pack(this, buffer, pos, comm)
    ...
    call this%rate_update_A%bin_pack(buffer, pos)
    call this%rate_update_B%bin_pack(buffer, pos)
  ...
  subroutine bin_unpack(this, buffer, pos, comm)
    ...
    call this%rate_update_A%bin_unpack(buffer, pos)
    call this%rate_update_B%bin_unpack(buffer, pos)
...
end module pmc_unit_test_rxn_first_order_loss
