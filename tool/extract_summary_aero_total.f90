...
!
! Read a NetCDF summary file and write out the time-resolved total
! number and mass concentration in text format.

program extract_summary_aero_total
  ...
  integer :: ios, i_time, i_radius, i_spec, i_unit_num, i_unit_mass
  real*8 :: num_conc, mass_conc

  ! process commandline arguments
  if (iargc() .ne. 2) then
     write(6,*) 'Usage: extract_summary_aero_total <netcdf_filename> <output_filename>'
     call exit(2)
  endif
  ...
  write(*,*) "  A(i, 2) = number concentration at time(i) (#/m^3)"
  write(*,*) "  A(i, 3) = mass concentration at time(i) (kg/m^3)"
  ...
  do i_time = 1,n_time
     ...
     write(out_unit, '(3e30.15e3)') time(i_time), &
          num_conc, mass_conc
  end do
  close(out_unit)
...
end program extract_summary_aero_total
